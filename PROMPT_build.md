...
0c. For reference, the output location is `src/*`.
...
4. After implementing, run validation: `node --check src/*.js` for syntax, `node --test src/**/*.test.js` for tests. Fix any failures before committing.
...
After committing, check @IMPLEMENTATION_PLAN.md. If **every item is `[x]` checked** and all tests pass and the game runs correctly (`node src/index.js` launches without error), then create the file `.ralph-complete` with a short summary of what was built. This signals the loop to stop.
